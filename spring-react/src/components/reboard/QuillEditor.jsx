...
const QuillEditor = ({value, handleContent, quillRef}) => {
    ...
    //내용 작성하고 내용이 변경될 때 마다 상태값이 바뀌면 화면을 새로 렌더링한다. -useState
    //같은 역할을 수행하는 imageHandler함수가 매번 새로 만들어진다.(비효율적이다.)
    //리액트 - 재조정 - useMemo(필드를 메모이제이션),  useCallback(함수를 메모이제이션)
    ...
    const imageHandler = useCallback(() => {
        ...
        const input = document.createElement("input"); // input 태그를 동적으로 생성하기 - 자바스크립트 사용함. - DOM API
        //8000번으로 요청을 보낸다.
        ...
        input.onchange = async () => {
            try{
                ...
                formData.append("image", file); // 위에서 만든 폼데이터에 이미지 추가 400 Bad Request
